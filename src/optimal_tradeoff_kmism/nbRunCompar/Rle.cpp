...
int RleText::AddLetterBegin(int pos, int period, int letter) {
	size_ ++;
	if (rle_[period].size() == 0) {
		...
		int c = rle_[period].front()->GetChar();
		if (c == letter) {
...
int Rle::AddLetterEnd(int pos, int period, int letter) {
	size_ ++;
	if (rle_[period].size() == 0) {
		RunRle *run = new RunRle(letter);
		...
		int c = rle_[period].back()->GetChar();
		if (c == letter) {
...
int RleText::AddLetterEndTSec(int pos, int period, int letter) {
...
void Rle::MakeRle(int32_t size_pattern, int *pattern, int32_t size_total) {
	int p, i = 0;
...
void RleText::MakeRle(int32_t size_pattern, int32_t size_text, int *text,
...
void Rle::DoString(int32_t *size, int **str) const {
	(*size) = size_;
	(*str) = new int[size_];
	int pos = 0;
	int letter;
...
void RleText::DoString(int32_t *size, int **str) const {
	(*size) = size_;
	(*str) = new int[size_];
	int pos = 0;
	int letter;
...
void ComputeResStar(int* t_star, int32_t size_p_star, int* p_star,
...
void Small8kPeriod(int32_t size_text, int *text, int32_t size_pattern,
 ...
 //    for (int i = 0; i < freq.size(); ++i)
    ...
    InitTabZeros(size_res_star, res_hd);

    // Frequent letters
    ComputeFreqHD(size_t_star, t_star, size_p_star,  p_star, &freq,
