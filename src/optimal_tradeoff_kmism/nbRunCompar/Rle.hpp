...
class Rle {
	...
	public:
		...
		int AddLetterEnd(int pos, int period, int letter);

		// Put the starting position of every run in the reading order
		void SetPositionRuns();

		// construct the RLE()
		void MakeRle(int32_t size_pattern, int *pattern, int32_t size_total);
		...
		void PrintRle() const;
		void DoString(int32_t *size, int **str) const;
};
...
class RleText : public Rle {
	...
	public:
		...
		//  return 1 if the function add a run, 0 else
		int AddLetterBegin(int pos, int period, int letter);
		int AddLetterEndTSec(int pos, int period, int letter);
		...
		void MakeRle(int32_t size_pattern, int32_t size_text, int *text,
