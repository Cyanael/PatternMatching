...
// int CharMap(int letter, int *map) {
//     return map[letter]);
// }
...
void SortFreqCaract(int32_t size_pattern, int *pattern, int k_nb_letters,
            float limit, int *map, int size_alphabet, vector<int> *freq) {
    freq->clear();
...
void ComputeInfreqApprox(int32_t size_text, int *text, int32_t size_res,
  ...
  for (int32_t i = 0; i < size_text; ++i) {
    if (IsInfreq(map[text[i]], infreq)) {
      current_val = map[text[i]];
...
int findApproximatePeriod(int32_t size_pattern, int *pattern, int k_nb_letters,
  ...
  int * pattern2 = new int[size_pattern2];
