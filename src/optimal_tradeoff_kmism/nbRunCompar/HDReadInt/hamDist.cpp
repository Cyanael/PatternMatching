...
void ReadPattern(string file, int32_t *size_pattern, int **pattern) {
...
void SortfreqInfreqCaract(int32_t size_pattern, int *pattern, float limit,
						vector<int> *freq, vector<int32_t> *infreq) {
	for (int32_t i = 0; i < size_pattern; ++i) {
		if (infreq[pattern[i]].size() == 0) {
			vector<int32_t> v = {i};
			infreq[pattern[i]] = v;
		}
		else
			infreq[pattern[i]].push_back(i);
	}
...
void ComputeFreq(int32_t size_pattern, int32_t size_text, int32_t size_res,
				int *text, int *pattern, vector<int> *frequent,
...
void ComputeInfreq(int32_t size_text, int *text, int32_t size_res,
...
int main(int argc, char* argv[]) {
	...
	int32_t size_pattern, size_text, size_textTotal;
	int *pattern;
	...
	int nb_iter = ceil(float(size_textTotal) / float(size_pattern));
	int rest = size_textTotal % size_pattern;

	for (int i = 0; i < nb_iter; ++i) {
		if (i < nb_iter-2) {
			for (int32_t j = 0; j < size_pattern; ++j) {
				// stream_text.get(character);
				// text[size_pattern + j] = character;
				stream_text >> text[size_pattern + j];
			}
		}
		else {
			...
			for (int32_t j = rest; j < size_pattern; ++j) {
				text[size_pattern + j] = '$';
			}
			rest = 0;
		}
