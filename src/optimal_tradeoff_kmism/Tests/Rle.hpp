...
class RunRle {
	...
	public:
		...
		void ReduceRun(int val);
		void ExtendRunBegin(int pos);
		void ExtendRunEnd();
		void ExtendRunEnd(int32_t size);
...
class Rle {
	...
	public:
		...
		void MakePattern(int32_t size, int min);

		void UpdateRle(int pas_run);
...
class RleText : public Rle {
	...
	public:
		...
		void MakeText(int32_t size, int error);
