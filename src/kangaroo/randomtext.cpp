...
bool Usage() {
    cout << endl << "How to run: ./exec size_text size_pattern ratio nb_errors ";
...
void CreateTextPattern(int32_t size_text_pattern, char **text_pattern, float ratio) {
    (*text_pattern) = new char[size_text_pattern]();
    struct timespec ts;
    ...
    float rand;
    int32_t nb_a = 0, nb_b = 0;
    for (int32_t i = 0; i < size_text_pattern; ++i) {
        rand = random()%(int)(1/ratio);
        // cout << rand << endl;
        if (rand > 0) {
            nb_a++;
            (*text_pattern)[i] = 'a';
...
int main(int argc, char* argv[]) {
    ...
    for (int i=0; i<nb_loops; ++i) {
        ...
        for (int j = 0; j < size_res; ++j) {
            curr_error = 0;
            pos_p = size_text;
            while (curr_error < nb_error_max && pos_p < (size_text + size_pattern)) {
    			if (text_pattern[pos_p]!=text_pattern[j]){
    				curr_error++;
    			}
    			pos_p++;
    		}
