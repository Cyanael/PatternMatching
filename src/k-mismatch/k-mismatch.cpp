...
//  add the bottom corners that influence delta
    ...
    c.influence = false;
    corners.push_back(c);
    //  this also works for MR being a 1 x d or d x 1 line and a 1 x 1 square
  }
}
...
//  create a dummy corner at the end of the text run
//  to indicate the end of the corners
  Corner dummy_c;
  dummy_c.y = 0;
...
void create_i_star_list(int r) {
  priority_queue<i_star_instance, vector<i_star_instance>,
                  Compare_i_star> tempq;
...
//      compute the new values
        ham.previous = (corner_diagonal - pos - 1) * delta.current
                        + ham.current;
...
//  Transform new_runs into corners
//  (can do this as we are in the offline setting)
  for (int r=0; r < approximate_period; r++) {
    for (int s = 0; s < approximate_period; s++) {
      add_corners(r, s);
    }
...
void print_setup_phase() {
  cout << "TEXT ENCODING AFTER SETUP PHASE: " << endl;
  for (int i = 0; i < approximate_period; i++) {
...
void print_text_RLE() {
  cout << "TEXT ENCODING AFTER BOTH ENCODINGS: " << endl;
  for (int i = 0; i < approximate_period; i++) {
    for (int j = 0; j < text_runs_length[i]; j++) {
      cout << "(" << text_RLE[i][j].position << "," << text_RLE[i][j].label;
...
void print_position_delta_hamming() {
  for (int r = 0; r < approximate_period; r++) {
    ...
    for (int s = 0; s < approximate_period; s++) {
      ...
      cout << instance_A_RLE[r][s]->delta.current << endl;
    }
  }
}
//******************************************************************************
...
//  We compute T_R taking advantage the input is offline
