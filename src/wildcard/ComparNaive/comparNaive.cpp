...
bool Usage(int argc){
	if (argc < 4){
		cout << endl << "How to run: ./exec text_file pattern_file patter_size -o optionalOutput";
...
void readPattern(string file, int32_t *size_pattern, double **pattern, unsigned int *size_fft){
...
void readRandomPattern(int32_t size_pattern, double *pattern, int32_t size_patternAll, double *patternAll) {
    struct timespec ts;
    ...
    int32_t begin = random()%(size_patternAll - size_pattern);
    for (int32_t i = 0; i < size_pattern; ++i){
        pattern[i] = patternAll[begin + i];
    }
}
...
int main(int argc, char* argv[]) {
	...
	unsigned int size_fft, size_text;
	...
	*/
	double *pattern, *patternAll;
	int32_t size_patternAll;

	// Open and read the file containing the pattern
	readPattern(file_pattern, &size_patternAll, &patternAll, &size_fft);

	pattern = new double[size_pattern]();
	...
	for (int i = 0; i < loop; ++i) {
		...
		readRandomPattern(size_pattern, pattern, size_patternAll, patternAll);
	    ...
	    int *res_naive = new int[size_res]();
